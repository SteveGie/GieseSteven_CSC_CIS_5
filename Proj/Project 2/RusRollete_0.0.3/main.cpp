 ...
 * Ver: 0.0.3, Implementing com players, this gonna be difficult.
...
bool satta(int&, bool, bool&, string, string);//Function only for single player... grammer reasons, plus i wanted it to feel a little different in single player.
void winSeq();
...
void p1game(){ //for now, no opponents.
    ...
    bool pdie=true, c1die=true, c2die=true;//__die = on death, equals false
    char sel, cont;
    int bull, crand; //may make separate int for cpu so they dont get have unfair knowledge
    float shoot, spin;
    ...
    while(pdie){
        ...
        if(sel=='a'||sel=='A'){
            ...
            cin>>choose;
            while(choose!=com1&&choose!=com2){
                cout<<"Invalid name, choose "<<com1<<" or "<<com2;
                cin>>choose;
            }
            if(choose==com1){
                cout<<"You pointed the gun at "<<com1<<" and squeezed the trigger...\n";
                if(bull==CHAMB){
                    cout<<"BOOOM!... sorry "<<com1<<" its just how the game is.\n";
                    bull=(rand()%(6-1+1))+1;
                    c1die=false;
                }
                else{
